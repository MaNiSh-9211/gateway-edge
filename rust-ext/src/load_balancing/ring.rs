...
pub fn build_weight_ring(upstreams: &[Upstream]) -> Vec<usize> {
    let total: usize = upstreams.iter().map(|u| u.weight.max(1)).sum();
    let mut ring = Vec::with_capacity(total);
    for (i, u) in upstreams.iter().enumerate() {
        let w = u.weight.max(1);
        for _ in 0..w {
            ring.push(i);
        }
    }
...
mod tests {
    ...
    fn weighted_ring_length() {
        let upstreams = vec![
            Upstream { name: "a".into(), address: "a:8080".into(), weight: 10 },
            Upstream { name: "b".into(), address: "b:8080".into(), weight: 5 },
        ];
    ...
    fn zero_weight_treated_as_one() {
        let upstreams = vec![
            Upstream { name: "a".into(), address: "a:8080".into(), weight: 0 },
            Upstream { name: "b".into(), address: "b:8080".into(), weight: 0 },
        ];
